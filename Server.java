...
public class Server {
    ...
    public static void main(String[] args) throws Exception {
		...
		System.out.println("Using Default Port Number 9899 \n");
    ...
    private static class ServerThread extends Thread {
	 ...
	 // list to keep track of files getting editied ...
            ...
            try {
				...
				if(!input.equals("server"))
				{
					...
					if(password.equals("NULL"))
						...
						if(authenticated==1){
						    out.println(newUserName+"  SERVER1: Please enter your choice 1.Read the file 2. Write to file 3. Delete from file  Type exit to quit. ");
							while (authenticated==1) {
									...
									else if(input.equals("1")) // Read from file 
									{
										...
										{
											out.println("Server under Synchronization Sorry try to read later");
										}
										else if(list.contains(filename))
										{
											out.println("File is being edited by the owner Sorry try again later");
											 ...
											 fr=new FileReader("./Server1/"+filename+ ".txt"); // userfilename
									...
									else if(input.equals("2")||input.equals("3")) // Write to file
									{
									   filedatashouldexchange=1;
                                        ...
                                        try
                                        {
                                                fr=new FileReader("./Server1/"+newUserName + ".txt"); // userfilename
                                                br=new BufferedReader(fr);
											...
											PrintWriter writer = new PrintWriter("./Server1/"+newUserName + ".txt");
						...
						// files need to be exchanged now			
						...
						out.println("done");
							...
							out.println("done");
								...
								String fileData=in.readLine();// get the file data
										PrintWriter writer = new PrintWriter("./Server1/"+filename+ ".txt");
		...
		private void writeToUserFile(String userName,String Password)
		{
			try
			{
				...
				FileWriter fw;
				fw=new FileWriter("account1.txt", true);
				fw.write(userName+" "+Password);//appends the string to the file
				fw.write("\n");
				fw.close();
				// synchronize new user credentials with other server 
				// This code is not unique in both server code as port changes 
				// this connects to other server on 9898
		...
		private String newUser(String userName) {
            ...
            try
            {
                ...
                fr=new FileReader("account1.txt");
                br=new BufferedReader(fr);
		...
		private void connectToServer(String uname,String password) throws IOException {
			...
			log("Came to connectToServer function \n now trying to connect to 9898 ");
		     Socket socket = new Socket("ranger1.cs.mtsu.edu", 9898); // server 2 port no server 2 is alwaya running on ranger 1 and server 1 on ranger 0 
			 BufferedReader in;
			 PrintWriter out;
			...
			out.println("server");
				...
				System.out.println(temp);
				//log("password sendint to server 2 is "+ password +"\n");
		...
		Socket socket = new Socket("ranger1.cs.mtsu.edu", 9898); // server 2 port no server 2 is alwaya running on ranger 1 and server 1 on ranger 0 
			...
			String s;
                            		    String fulldata="";
											...
											FileReader fr;
											BufferedReader br;
                                                fr=new FileReader("./Server1/"+uname + ".txt"); // userfilename
                                                br=new BufferedReader(fr);
	...
	try{
           File permfile = new File("Server1",userName + ".txt");
